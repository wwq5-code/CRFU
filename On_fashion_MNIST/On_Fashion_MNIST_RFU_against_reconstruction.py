...
def unlearning_frkl(vibi_f_frkl, optimizer_frkl, vibi, epoch_test_acc, dataloader_erase, dataloader_remain, loss_fn,
    ...
    for epoch in range(args.num_epochs):
        ...
        for (x, y), (x2, y2) in zip(dataloader_erase, dataloader_remain):
            ...
            unlearning_item = args.kld_r * KLD_mean.item() - args.unlearn_bce_r * BCE.item() + args.unlearn_ykl_r * kl_f_e.item() -  unl_rate * H_p_q.item() - args.reverse_rate * (log_z.item() + log_y.item())

            unl_y_item = args.unlearn_learning_rate * H_p_q.item()
            except_unl_y = unlearning_item + args.unlearn_learning_rate * H_p_q.item()
            ...
            if unlearning_item < 0:
                unl_rate = args.unlearn_learning_rate #except_unl_y/(H_p_q.item()+1)
            else:
                unl_rate = args.unlearn_learning_rate

            nips_unl_item = args.kld_r * KLD_mean.item() + args.unl_r_for_bayesian * (- H_p_q.item()) - args.reverse_rate * (log_z.item() + log_y.item())
            nips_except_unl_y = nips_unl_item + args.unl_r_for_bayesian * H_p_q.item()

            if nips_unl_item < 0:
                nips_unl_rate = args.unl_r_for_bayesian #nips_except_unl_y/(H_p_q.item()+1)
            else:
                nips_unl_rate = args.unl_r_for_bayesian
            ...
            if train_type == 'VIBU':
                ...
                loss = args.kld_r * KLD_mean + nips_unl_rate * (- H_p_q) - args.reverse_rate * (log_z + log_y)
...
args.reverse_rate = 0.1*args.unlearn_learning_rate
args.kld_r = 0.1*args.unlearn_learning_rate #1
args.unlearn_ykl_r = args.unlearn_learning_rate*0.0004
args.unlearn_bce_r = args.unlearn_learning_rate*0.0001
...
if args.dataset == 'MNIST':
    ...
    train_set = FashionMNIST('../../data/mnist', train=True, transform=transform, download=True)
    test_set = FashionMNIST('../../data/mnist', train=False, transform=transform, download=True)
